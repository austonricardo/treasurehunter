...
//   gcc -Wall -Wno-unused-function -O2 --std=c99 -march=native -I src/ - I unistd/ -I ./ tho.c -lgmp -o thowin.exe
//   gcc -Wall -Wno-unused-function -O2 --std=c99 -march=native -I src/ -I ./ tho.c -lgmp -o thowin.exe
...
#define bzero(b,len) (memset((b), '\0', (len)), (void) 0) //on windows
...
int main(int argc, char **argv) {
    unsigned int bmul_size  = ( argc > 1 ? atoi(argv[1]) : 10 );    // ecmult_big window size in bits //20
    unsigned int batch_size = ( argc > 2 ? atoi(argv[2]) : 8 );    // ecmult_batch size in keys //16
    ...
    // Let us create three threads linux
    /*
    pid_t pid[pool_size];
    for (int i = 1; i < pool_size; i++){ 
        args->tid = i;
        pid[i] = fork(); //linux
        //pid[i] = CreateProcess(); //Windows
        if (pid[i] < 0) {
            perror("fork error");
        } else if (pid[i] > 0) {
            //in parent process
        } else {
            procwork(args); //in child process
        }
    }
    if(pid[1] > 0){ //in parent process
        args->tid = 0;
        procwork(args);
    }
    */
   ...
   /*
    for (int i = 1; i < pool_size; i++){//need put on thread 
        args->tid = i;
        while(true){
            procwork(args);
        }
