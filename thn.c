...
//   gcc -Wall -Wno-unused-function -O2 --std=c99 -march=native -I src/ -I ./ thn.c -lgmp -lpthread -o thn
...
void * procwork(void *_args){
    // Store the value argument passed to this thread 
    tpargs *args = (tpargs *) _args;
...
int main(int argc, char **argv) {
    ...
    for (unsigned char i = 0; i < pool_size; i++){ 
        args->tid = i;
        pthread_create(&pthreads[i], NULL, procwork, (void *)args); 
        //procwork((void *)args);
    }
    for (unsigned char i = 0; i < pool_size; i++){ 
